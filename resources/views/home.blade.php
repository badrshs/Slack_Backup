...
<html lang="en">
	...
	<body>
		<div class="font-sans antialiased h-screen flex">
			<div
				class="bg-indigo-darker text-purple-lighter flex-none w-64 pb-6 hidden md:block"
			>
				<div class="text-white mb-2 mt-3 px-4 flex justify-between">
					<div class="flex-auto">
						...
						<h4>Total messages backed up</h4>
						<span
							style="
								padding: 10px;
								font-size: 24px;
							...
							"
						>
							{{\App\Conversations::all()->count()}}
							<small style="font-size: 50%">messages</small>
						</span>
						<br />
						<br />
						<?php  ?>
				...
				<div class="mb-8 boxChild" style="height: 275px; overflow-y: scroll">
					@foreach($channels as $channel) @if(!$channel->has_auth ||
					!$channel->is_channel) @continue @endif
					<div
						style="margin-left: 5px"
						class="py-1 channel_list @if($current_channel->id==$channel->id) bg-teal-dark @endif"
						@if($current_channel-
					>
						id==$channel->id) id="scroll-to" @endif >
						@if(sizeof($channel->chats)==0)
						<a
							href="{{route('backup.channel',$channel->id)}}"
							class="button px-1"
						...
						@endif
						<a
							href="{{route('channel',$channel->id)}}"
							class="py-1 px-1 text-white"
				...
				<div class="mb-8 boxChild" style="height: 350px; overflow-y: scroll">
					...
					@foreach($channels as $channel) @if(!$channel->has_auth |
					!$channel->is_direct) @continue @endif
					<div
						class="py-1 channel_list @if($current_channel->id==$channel->id) bg-teal-dark @endif"
						@if($current_channel-
					>
			...
			<div class="flex-1 flex flex-col bg-white overflow-hidden">
				...
				<div class="border-b flex px-6 py-2 items-center flex-none">
					...
					<div class="ml-auto hidden md:block">
						<a
							href="{{route('backup.channel',$current_channel->id)}}"
							class="button px-1"
					...
					<div class="ml-auto hidden md:block">
						<div class="relative">
							...
							/>
							<div
								class="absolute pin-y pin-l pl-3 flex items-center justify-center"
							>
								<svg
						...
						<div class="flex-1 overflow-hidden">
							<div>
								...
								<span class="text-grey text-xs"
									>{{date('m-d-Y h:i',$message->ts)}}</span
								>
							...
							<p class="text-black leading-normal text-message">
								{{$message->text}}
							</p>
						</div>
					</div>
